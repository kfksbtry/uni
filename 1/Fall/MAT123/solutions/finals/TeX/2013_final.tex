...
\begin{center}
\textbf{2012-2013 Fall \\MAT123-[Instructor02]-02, [Instructor05]-05, Final\\(23/01/2013)}
\end{center}

\textbf{1.} Determine whether each given sequence with $n$th term converges or diverges. Evaluate the limit of each convergent sequence. Explain all your work, and write clearly.

\textbf{(a)} $\displaystyle a_n=(-1)^n\,n\sin\left(\frac1n\right)$ \ \ \ \textbf{(b)} $\displaystyle a_n= e^{\cos\left(\textstyle\frac1n\right)}$

\vspace{1em}

\textbf{2.} Determine whether the following series converge or diverge. Give reasons for your answers.

\textbf{(a)} $\displaystyle\sum_{n=1}^\infty\frac n{\left(3+n^2\right)^{3/4}}$ \ \ \ \textbf{(b)} $\displaystyle\sum_{n=2}^\infty\frac1{\sqrt n}\ln\left(\frac{n+1}{n-1}\right)$ \ \ \ \textbf{(c)} $\displaystyle\sum_{n=1}^\infty\frac{(2n)!}{5^n\left(n!\right)^2}$ \ \ \ \textbf{(d)} $\displaystyle\sum_{n=1}^\infty\frac1{n^2} e^{1/n}$

\vspace{1em}

\textbf{3.} Integrate the following functions and write each step in detail.

\textbf{(a)} $\displaystyle\int\frac{dx}{ e^x+1}$ \ \ \ \textbf{(b)} $\displaystyle\int x\arcsin x\,dx$

\vspace{1em}

\textbf{4.} Find the length of the curve $\displaystyle y=\int_0^x\sqrt{\cos\left(4t\right)}\,dt$ for $0\leq x\leq \pi/8$.

\vspace{1em}

\textbf{5.} For the function $\displaystyle f(x)=\frac x{x^2-4}$,

\textbf{(a)} Find all the asymptotes of $f$.

\textbf{(b)} Find the intervals of increase or decrease.

\textbf{(c)} Find the local maximum and minimum values, if any.

\textbf{(d)} Find the intervals of concavity and the inflection points, if any.

\textbf{(e)} Sketch the graph of $f$.

\newpage

\begin{center}
\textbf{2012-2013 Fall Final (23/01/2013) Solutions\\
(Last update: 30/08/2025 00:29)}
\end{center}

\textbf{1. (a)} Evaluate the limit of the non-alternating part at infinity.
...
\textbf{(b)} The exponential function $ e^x$ and the trigonometric function $\cos x$ are continuous everywhere.
...
\textbf{2. (a)} Let $a_n=f(n)$. Define $\displaystyle f(x)=\frac x{\left(3+x^2\right)^{3/4}}$. The function is continuous for $x>1$ because the numerator and the denominator are continuous for $x>1$ and $\left(3+x^2\right)^{3/4}\neq0,\:\forall x\in\mathbb{R}$.
...
\textbf{(b)} $\ln(1+x)<x$ for $x>-1$. Therefore,
...
$b_n$ converges by the $p$-series test because $3/2>1$. Since $b_n$ converges, by the Direct Comparison Test, $a_n$ also converges.

\textbf{(c)} Apply the Ratio Test. Let $\displaystyle a_n=\frac{(2n)!}{5^n\left(n!\right)^2}$.
...
By the Ratio Test, the series converges absolutely. Since the series converges absolutely, the series converges.

\textbf{(d)} Let $\displaystyle a_n=\frac1{n^2} e^{1/n}$. Define $\displaystyle f(x)=\frac1{x^2} e^{1/x}$. The function is continuous for $x>1$ because the expressions $\displaystyle\frac1{x^2}$ and $ e^{1/x}$ are continuous for $x>1$ and $x^2\neq0,\:\forall x>1$.
...
\textbf{3. (a)} Add and subtract $ e^x$ in the numerator.
...
\[x-\int\frac{du}{u}=x-\ln|u|+c=\boxed{x-\ln\left| e^x+1\right|+c=x-\ln\left( e^x+1\right)+c,\:c\in\mathbb{R}}\:\left[ e^x+1>0\right]\]

\textbf{(b)} Solve the integral using the method of integration by parts.
...
Now, we need to find the result of the integral on the right. Let $x=\sin u$ for $\displaystyle\frac\pi2<u<\frac\pi2$. Then $dx=\cos u\,du$.
...
\textbf{4.} The length of a curve defined by $y=f(x)$ whose derivative is continuous on the interval $a\leq x\leq b$ can be evaluated using the integral.
...
\textbf{5. (a)} Find the horizontal asymptotes.
...
\textbf{(b)} Compute the first derivative and set it to $0$ to find the critical points. Apply the product rule appropriately.
...
\textbf{(c)} $\boxed{\text{No local maximum or minimum values exist.}}$

\textbf{(d)} Compute the second derivative.
