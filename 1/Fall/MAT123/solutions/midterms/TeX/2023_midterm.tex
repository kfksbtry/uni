...
\begin{center}
\textbf{2023-2024 Fall \\MAT123-02,05 Midterm\\(01/11/2023)}
\end{center}

\textbf{1.} Evaluate the following limits without using L'Hôpital's rule.

\textbf{(a)} $\displaystyle \lim_{t\to 0} \frac{\tan^{-1} t}{\sin^{-1} t}$ \ \ \ \textbf{(b)} $\displaystyle \lim_{x\to 0^+} x e^{\displaystyle \sin(1/x)}$

\vspace{1em}

\textbf{2.} A spherical balloon is being filled with air in such a way that its radius is increasing at the constant rate of 2 cm/s. At what rate is the volume of the balloon increasing at the instant when its surface has area $4\pi$ cm$^2$?

\vspace{1em}

\textbf{3. (a)} State MVT.

\textbf{(b)} Using MVT, show that for all $x>0$,
...
\textbf{5.} Find the number $A$ so that $\displaystyle\lim_{x\to\infty}\left(\frac{x+A}{x-2A}\right)^{x}=5$.

\vspace{1em}

\textbf{6.} Sketch the graph of $f(x)=(\ln x)^2$.

\newpage

\begin{center}
\textbf{2023-2024 Fall Midterm (01/11/2023) Solutions\\
...
\textbf{2.} Let $V(t),\,S(t),\,r(t)$ represent the volume, surface area and radius, respectively. $r'(t) = 2$ for all $t$. The rate of change of volume at $t=t_0$ is
...
\textbf{3. (a)} The MVT states that if a function $f$ is continuous on $[a,b]$ and differentiable on $(a,b)$, there is at least one point $P$ on the interval $(a,b)$ such that the slope of the line that passes through the endpoints is equal to the slope of the line that is tangent to $P$.

\textbf{(b)} Let $f(x)= e^x$. $f$ is continuous on $[0,x]$ and differentiable on $(0,x)$.  There exists at least one point $c$ on $(0,x)$ such that
...
\textbf{4.} Let us find the derivative of $x^x$ with respect to $x$.
...
Evaluating $y'$ at $(1,1)$ gives $\displaystyle-\frac23$. Using the straight line formula $y-y_0 = m(x-x_0)$, we get

\[\boxed{y-1 =-\frac23(x-1)}\]

\newpage

\textbf{5.} Take the logarithm of both sides of the equation and apply L'Hôpital's rule.
...
\textbf{6.} First off, find the domain. The expression is defined \textit{only} for $x>0$. The only vertical asymptote occurs at $x=0$.
