...
\textbf{1.} Let $f(x,y)=6-x^2-y^2$. Find the tangent plane and normal line equations (symmetric or parametric equations) of the graph of $f$ at the point $P(1,2,1)$.

\vspace{1em}

\textbf{2.} Find all critical points of the given function $f(x,y)=x^3+y^3-3xy+2$ and classify them (i.e., determine whether each critical point corresponds to a local maximum, local minimum, or saddle point).

\vspace{1em}

\textbf{3.} Find the maximum and minimum of the function $f(x,y)=81x^2+y^2$ subject to the given constraint $4x^2+y^2=9$ using Lagrange multipliers.

\vspace{1em}

\textbf{4.} The equation $z^3-zy^2+yx=3$ defines $z$ implicitly as a function of $x$ and $y$. It is given that $z=2$ when $(x,y)=(-3,1)$. Evaluate $\displaystyle\frac{\partial z}{\partial x}$ and $\displaystyle\frac{\partial z}{\partial y}$ at $(-3,1)$.

\vspace{1em}

\textbf{5. (a)} Reverse the order of integration and evaluate the double integral.

\[\int_0^{\sqrt2}\int_0^{2-x^2}x{e}^{x^2}\,dy\,dx\]
 
\textbf{(b)} Evaluate the double integral
...
\textbf{6. (a)} Convert the triple integral into cylindrical coordinates. Do not evaluate the integral.

\[\int_{-1}^1\int_0^{\sqrt{1-y^2}}\int_{x^2+y^2}^{\sqrt{x^2+y^2}}xyz\,dz\,dx\,dy\]

\textbf{(b)} Use a triple integral in spherical coordinates in order to find the volume of the sphere centered at $(0,0,0)$ with radius $3$.

\vspace{1em}

\textbf{7. (a)} Determine whether the vector field $\mathbf{F}(x,y,z)=y\,\mathbf{i}+x\,\mathbf{j}+z^2\,\mathbf{k}$ is conservative and find a potential if it is conservative.

\textbf{(b)} Evaluate the line integral $\int_C\mathbf{F}\cdot d\mathbf{r}$ for the vector field $\mathbf{F}(x,y,z)=z\,\mathbf{i}-y\,\mathbf{j}+2x\,\mathbf{k}$ along the curve $x=t,\:y=t^2\:z=t^3$ from $(0,0,0)$ to $(1,1,1)$.

\newpage

\begin{center}
\textbf{2024-2025 Spring Final (18/06/2025) Solutions\\
(Last update: 08/08/2025 00:21)}
\end{center}

\textbf{1.} The equation of the tangent plane at a point on the surface and the normal line to this plane are given, respectively, by
...
Using $(1)$ and $(2)$ determine the equations.
...
\textbf{2.} To identify the critical points, find where both $f_x=f_y=0$ or one of the partial derivatives does not exist.
...
\textbf{3.} Let $g(x,y)=4x^2+y^2-9$ be the constraint. Solve the system of equations below.
...
Evaluate $f$ at the points $\displaystyle(0,3),\:(0,-3),\:\left(\frac32,0\right),\:\left(-\frac32,0\right)$ and compare them all.
...
\textbf{4.} Obtain the partial derivatives of both sides.
...
\begin{align*}{I}&=\int_0^{\sqrt2}\int_0^{2-x^2}x{e}^{x^2}\,dy\,dx=\int_0^2\int_0^{\sqrt{2-y}}x{e}^{x^2}\,dx\,dy\\\\&=\int_0^2\left[\frac{{e}^{x^2}}2\right]_{x=0}^{x=\sqrt{2-y}}\,dy=\frac12\int_0^2\left({e}^{2-y}-1\right)\,dy\\\\&=\frac12\left[-{e}^{2-y}-y\right]_0^2=\boxed{\frac{{e}^2-3}2}\end{align*}
\end{minipage}
\end{center}
...
Notice that it would be difficult to solve the integral in rectangular coordinates. We may switch to polar coordinates to easily evaluate the integral.

\[x=r\cos\theta,\quad y=r\sin\theta,\quad x^2+y^2=r^2,\quad dA=r\,dr\,d\theta\]

\begin{align*}{I}&=\iint_R\frac{2xy}{x^2+y^2}\,dA=\int_0^{\textstyle\frac\pi2}\int_0^a\frac{2r\cos\theta\cdot r\sin\theta}{\left(r\cos\theta\right)^2+\left(r\sin\theta\right)^2}\,r\,dr\,d\theta\\\\&=\int_0^{\textstyle\frac\pi2}\int_0^a\frac{2r^3\cos\theta\sin\theta}{r^2(\sin^2\theta+\cos^2\theta)}\,dr\,d\theta=\int_0^{\textstyle\frac\pi2}\int_0^a r \sin(2\theta)\,dr\,d\theta=\int_0^{\textstyle\frac\pi2}\sin(2\theta)\left[\frac{r^2}2\right]_{r=0}^{r=a}\,d\theta\\\\&=\frac{a^2}2\int_0^{\textstyle\frac\pi2}\sin(2\theta)\,d\theta=\frac{a^2}2\left[\frac{-\cos(2\theta)}2\right]_0^{\textstyle\frac\pi2}=\frac{a^2}4\left(-\cos\pi+\cos0\right)=\boxed{\frac{a^2}2}\end{align*}

\vspace{1em}

\textbf{6. (a)} Use the transformation below.
\[
\begin{array}{c}
z=z\\
x=r\cos\theta\\
...
\textbf{(b)} We have the sphere $x^2+y^2+z^2=9$. Use the transformation below.
...
\textbf{(b)} Parametrize the curve and then evaluate the integral.
