...
\textbf{1.} Consider the curve with the polar equation $r=2\cos(3\theta)$.

\textbf{(a)} Find symmetry properties for the given curve and sketch the curve.

\textbf{(b)} Find the area of the region enclosed by the curve.

\vspace{1em}

\textbf{2.} Identify (describe and sketch) the following curves with polar equations using Cartesian coordinates.

\textbf{(i)} $r=2\sin\theta +2\cos\theta$ \ \ \ \textbf{(ii)} $r=\tan\theta\sec\theta$

\vspace{1em}

\textbf{3. (a)} The following vectors are given.
...
\textbf{(i)} $\mathbf{u}$ and $\mathbf{v}$ are perpendicular vectors. \ \ \ \ \ \textbf{(ii)} $\mathbf{u}$ and $\mathbf{v}$ are parallel vectors.

\vspace{0.5em}

\textbf{(b)} The points $A(5,1,3),\:B(3,1,5),\:C(5,3,5)$ are given. Show that the triangle $\triangle{ABC}$ is equilateral and find its area.

\textbf{(c)} Find the intersection of the line $L$ and the plane $R$.

\vspace{1em}

\textbf{4. (a)} Find an equation of the plane through the point $P(1,2,3)$ parallel to the plane $R: x+y+z=1$.

\textbf{(b)} Find the parametric equations of a line $L$ through the point $P(1,2,3)$ and perpendicular to the plane $R$.

\vspace{1em}

\textbf{5.} Evaluate the limits, if they exist, and explain your answer.

\textbf{(a)} $\displaystyle\lim_{(x,y)\to(0,0)}\frac{x^3y}{x^6+y^2}$ \ \ \ (b) $\displaystyle\lim_{(x,y)\to(0,0)}x^4\sin\left(\frac1{x^2+\left|y\right|}\right)$ \ \ \ 

\vspace{1em}

\textbf{6. (a)} Find a vector function that represents the curve of intersection of two cylinders $x^2+y^2=1$ and $z=4x^2$.

\textbf{(b)} Find the unit tangent vector to the curve in part (a) at $\displaystyle t=\frac\pi2$.

\textbf{(c)} Find a formula for the length of the curve in part (a). Do not evaluate the length.

\vspace{1em}

\textbf{7.} Use traces to sketch and identify the surface given by the equation $z=-x^2-y^2+2$.

\newpage

\begin{center}
\textbf{2024-2025 Spring Midterm (09/04/2025) Solutions\\
(Last update: 29/08/2025 01:59)}
\end{center}

\textbf{1. (a)} Determine whether the graph is symmetric about the $x$-axis.
...
\textbf{(b)} It is sufficient to calculate the area of the upper half of the leaf right to the $y$-axis and multiply the result by six.
...
\textbf{2. (i)} Multiply each side by $r$.
...
\textbf{(ii)} Rearrange the equation.
...
\textbf{3. (a) (i)} If $\mathbf u$ and $\mathbf v$ are perpendicular, the dot product of these vectors is equal to zero.
\[\mathbf u\cdot\mathbf v=\left\langle6,-3,2\right\rangle\cdot\left\langle4p+1,\,p-2,\,1\right\rangle=6(4p+1)-3(p-2)+2\cdot1=21p+14=0\implies \boxed{p=-\frac23}\]

\textbf{(ii)} If $\mathbf u$ and $\mathbf v$ are parallel, the cross product of these vectors is equal to the zero vector.
...
\textbf{(b)} The sides of an equilateral triangle have the same length.
...
\textbf{4. (a)} The planes have the same normal $\mathbf n=\left\langle1,1,1\right\rangle$. Using the definition $\mathbf n\cdot\overrightarrow{PP_0}=0$, we obtain
\[1(x-1)+1(y-2)+1(z-3)=0\implies\boxed{x+y+z=6}\]

\textbf{(b)} The direction vector of the line is the normal vector of the plane. Therefore, the parametric equations for the line $L$ is as follows.
...
\textbf{(c)} Substitute the equation of $L$ in the equation of the plane $R$.
...
\textbf{5. (a)} Apply the Two-Path Test.
...
\textbf{(b)} We have the inequality $\left|\sin\theta\right|\leq1$ for all values of $\theta$. Therefore,
...
\textbf{6. (a)} Parametrize the curve using $0\leq t\leq 2\pi$.
...
\textbf{(b)} The tangent vector can be obtained by taking the first derivative of the vector function.
...
\textbf{(c)} The length of the parametrized curve $\mathbf{r}(t)=\left\langle x(t),\,y(t),\,z(t)\right\rangle$ for $a\leq t\leq b$ can be evaluated using the integral
...
\textbf{7.} This is a circular paraboloid with the vertex $(0,0,2)$ opening downward along the $z$-axis.
