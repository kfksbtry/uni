...
\textbf{1.} Find an equation for the line $L$ that contains the point $P= (-1,3,1)$ and is orthogonal to the line
\[\frac{x-2}{-1}=\frac{y-1}{-2}=\frac{z-5}{1}=\lambda,\quad\lambda\in\mathbb{R}.\]

\vspace{1em}

\textbf{2.} Sketch the graph of the following surfaces.

\textbf{(a)} $z= e^y$ \ \ \ \textbf{(b)} $y=z^2-x^2$

\vspace{1em}

\textbf{3.} The position vector for a particle in space is given as
...
\textbf{4.} Let $f$ be a function defined by
\[
f(x,y) =
\left.
\begin{cases}
...
\textbf{5.} Use the $\epsilon - \delta$ definition and show that the function
\[
f(x,y) =
\left.
\begin{cases}
...
\textbf{6.} Note that in Cartesian coordinates, the Cauchy-Riemann equations are
...
\textbf{7.} According to Poiseuille’s law, the resistance to the flow of blood offered by a cylindrical blood vessel of radius $r$ and length $x$ is
...
\textbf{8.} Find the critical points of $f(x, y)=-x^3+9x-4y^2$ and classify each point as a relative maximum, a relative minimum, or a saddle point.

\newpage

\begin{center}
\textbf{2019-2020 Spring Midterm (08/06/2020) Solutions\\
(Last update: 28/08/2025 16:57)}
\end{center}

\textbf{1.} Let $M$ be the line $\dfrac{x-2}{-1}=\dfrac{y-1}{-2}=\dfrac{z-5}{1}=\lambda,\quad\lambda\in\mathbb{R}$. The direction vector of $M$ is $\mathbf u=\left\langle-1,-2,1\right\rangle$.
...
\textbf{3.} The velocity and acceleration vectors can be obtained by taking the first and the second derivatives of the vector function with respect to the parametrization variable, respectively.
...
\textbf{4.} Apply the Two-Path Test.
...
\textbf{5.} The value of the function at the point $(0,0)$ is $0$. Therefore, we will show that the limit $L$ is also $0$. For every $\epsilon>0$, there exists a $\delta>0$ such that
...
\textbf{6.} $u$ and $v$ are functions of $x$ and $y$. $x$ and $y$ are functions of $r$ and $\theta$. Use the chain rule.
...
\textbf{7.} The total differential of $R$ is
