...
\textbf{(a)} $y=\ln z$ \ \ \ \textbf{(b)} $x^2+2y^2-3z^2+1=0$

\vspace{1em}

\textbf{2.} State the $\epsilon-\delta$ definition of the limit of a function of two variables, and using the $\epsilon-\delta$ definition, show that
\[\lim_{(x,y)\to(0,0)}\left(2x^2+y^2\right)=0\]

\vspace{1em}

\textbf{3.} For $t\in\mathbb{R}$, let $l_1$ and $l_2$ be two lines given by
\[\begin{array}{ccccc}
x=3t&&&x=1+6\lambda&\\
...
\textbf{(i)} Evaluate the partial derivatives $f_x(0,0)$ and $f_y(0,0)$.

\textbf{(ii)} Show that $f(x,y)$ is not differentiable at $(0,0)$.

\vspace{1em}

\textbf{5.} Let $u=u(x,y)$ and $v=v(x,y)$. The Cauchy-Riemann equations are
...
\textbf{6.} Determine the direction at the point $(1,1)$ in which the rate of change of
\[f(x,y)=\frac{2^{xy}}x\]

is the largest. Compute this rate of change.

\vspace{1em}

\textbf{7.} Find all critical points of the function

\[f(x,y)=(x-1)(y+1)(x-y+3)\]

and then determine whether each critical point corresponds to a local maximum, a local minimum or a saddle point.

\newpage

\begin{center}
\textbf{2023-2024 Spring Midterm (26/04/2024) Solutions\\
...
\textbf{2.} The $\epsilon-\delta$ definition is the formal way of calculating the limit of a function at a point $(x_0,y_0)$. According to the definition, for every $\epsilon>0$, there exists a $\delta$ such that
...
\textbf{4. (i)} Use the definition of the partial derivative.
\begin{align*}f_x|_{(0,0)}&=\lim_{h\to0}\frac{f(0+h,0)-f(0,0)}h=\lim_{h\to0}\frac{\dfrac{(0+h)^2\cdot{e}^{(0+h)^2+0}}{(0+h)^2+0^2}}{h}=\lim_{h\to0}\frac{{e}^{h^2}}{h}\\\\&=\infty\:(f_x\:\text{does not exist})\end{align*}
\[f_y|_{(0,0)}=\lim_{h\to0}\frac{f(0,0+h)-f(0,0)}h=\lim_{h\to0}\frac{\dfrac{0^2\cdot{e}^{0^2+(0+h)}}{0^2+(0+h)^2}}{h}=\lim_{h\to0}\frac{0}{h}=0\]

\textbf{(ii)} $f_y$ is a finite number. However, since $f_x$ does not exist, $f$ is not differentiable at $(0,0)$.

\vspace{1em}

\textbf{5.} $u$ and $v$ are functions of $x$ and $y$. $x$ and $y$ are functions of $r$ and $\theta$. Use the chain rule.
...
\textbf{6.} The function $f$ has the maximum rate of change if the gradient vector of $f$ and the unit direction vector $\mathbf u$ are in the same direction. Apply the quotient rule to compute the gradient of $f$.
