...
void getOptions(Options *opts); // Details given in the function definition.
void printArray(WordFrequency *array, size_t length, Options opts); // Prints the array with the options.
void incrementFreq(WordFrequency **array, size_t *length, char *name, Options opts); // Increments the frequency of words.
void appendWord(WordFrequency **array, size_t *length, char *name, Options opts); // Handles the registry of words.
...
int main(void) {
	...
	if (inpf) {
		...
		char line[MAX_LINE_LENGTH] = "0", newLine[MAX_LINE_LENGTH] = "";
...
void getOptions(Options *opts) {
	...
	opts->sort = askForNumber(); // 1 for alphabetical, 2 for frequency sort.

	printf("%s", "Enter either 1 or 2 for the order of the sorting.\n\t1. Ascending\n\t2. Descending\n");
	opts->order = askForNumber(); // 1 for ascending, 2 for descending order.
	...
	opts->case_sensitive = askForNumber(); // Sensitive for 2, 1 otherwise.
}

void incrementFreq(WordFrequency **array, size_t *length, char *name, Options opts) {
...
void appendWord(WordFrequency **array, size_t *length, char *name, Options opts) {
	...
	*array = res;
	...
	(*array)[*length].word[strlen(name)] = '\0';
	(*array)[*length].frequency = 1; 	// Initialize the frequency of the new word to 1.
	(*length)++;
}

void printArray(WordFrequency *arr, size_t length, Options opts) {
