...
\textbf{3.} The following integral gives the volume of the region that lies inside the sphere $x^2+y^2+z^2=4$ and between the planes $z=0$ and $z=1$.
\[V=4\int_0^{\pi/2}\int_0^{\sqrt3}\int_0^1r\,dz\,dr\,d\theta+4\int_0^{\pi/2}\int_{\sqrt3}^2\int_0^{\sqrt{4-r^2}}r\,dz\,dr\,d\theta\]

\textbf{(a)} Write the integral in rectangular coordinates with the order of integration $dz\,dy\,dx$.

\textbf{(b)} Write the integral in spherical coordinates.

\vspace{1em}

\textbf{4. (a)} Is $\mathbf{F}(x,y)=2xy\sin\left(x^2y\right)\mathbf{i}-x^2\sin\left(x^2y\right)\mathbf{j}$ conservative? Why?
...
\textbf{5.} $\mathbf{F}(x,y,z)=\left(2x+y^2+z\cos x\right)\mathbf{i}+\left(2xy+{e}^z\right)\mathbf{j}+\left(1+y{e}^z+\sin x\right)\mathbf{k}$

\textbf{(a)} Let $C $ be the curve of intersection of the cone $z^2=4x^2+9y^2$ and the plane $z=1+x+2y$, and let $D$ be the part of the curve $C$ that lies in the first octant $x\geq0,\:y\geq0,\:z\geq0$ from $(1,0,2)$ to $(0,1,3)$. Evaluate $\int_D\mathbf{F}\cdot d\mathbf{r}$.
...
\begin{center}
\textbf{2023-2024 Fall Resit (29/01/2024) Solutions\\
(Last update: 07/08/2025 15:33)}
\end{center}

\textbf{1. (a)} To find the critical points of $f$, determine where both $f_x=f_y=0$ or one of the partial derivatives does not exist.
...
\textbf{(b)} Let $g(x,y,z)=x^2+4y^2+9z^2-1764$ be the constraint. Then solve the system of equations below.
...
From $\phi=0$ to $\displaystyle\phi=\frac\pi3$, the upper bound for $\rho$ is $\displaystyle\rho=\frac1{\cos\phi}$. From $\displaystyle\phi=\frac\pi3$ to $\displaystyle\phi=\frac\pi2$, it is $\rho=2$. The equivalent integral is as follows.
...
\textbf{4. (a)} For $\mathbf{F}$ to be conservative, it must be the gradient of some potential function $\phi$. We may apply the component test to determine whether the mixed partial derivatives are equal.
...
\begin{align*}\frac{\partial f}{\partial y}&=\frac{\partial}{\partial y}\left(x^2+xy^2+z\sin x+g(y,z)\right)=2xy+g_y(y,z)=2xy+{e}^z\implies g_y(y,z)={e}^z\end{align*}

\[\int\frac{\partial f}{\partial y}\,dy=\int\left(2xy+{e}^z\right)\,dy=x^2+z\sin x+xy^2+y{e}^z+h(z)=f(x,y,z)\]

\begin{align*}\frac{\partial f}{\partial z}&=\frac{\partial}{\partial z}\left(x^2+z\sin x+xy^2+{e}^z+h(z)\right)=\sin x+y{e}^z+h_z(z)\\\\&=1+y{e}^z+\sin x\implies h_z(z)=1\end{align*}
...
\begin{align*}\int_D\mathbf{F}\cdot d\mathbf{r}&=f(0,1,3)-f(1,0,2)={e}^3+3+c-(1+2\sin1+2+c)=\boxed{{e}^3-2\sin1}\end{align*}
...
\begin{align}{I}&=\oint_C\left(x^3\sin\left(\sqrt{x^2+4}\right)-x{e}^{x+2y}\right)\,dx + \left(\cos\left(y^3+y\right)-4y{e}^{x+2y}\right)\,dy\nonumber\\\nonumber\\&=\iint_R\left(\frac{\partial{F}_2}{\partial x}-\frac{\partial{F}_1}{\partial y}\right)\,dA=\iint_R\left(-4y{e}^{x+2y}\cdot1-\left(-x{e}^{x+2y}\cdot2\right)\right)\,dA\nonumber\\\nonumber\\&=\iint_R(2x-4y)\cdot{e}^{x+2y}\,dA\end{align}

From the edges of the parallelogram, we have $\displaystyle x=2y+2,\:x=2-2y,\:x=-2-2y,\:x=2y-2$. Now, use the method of change of variables. Let $\displaystyle u=x-2y,\:v=x+2y$. Then $\displaystyle x=\frac{u+v}2,\quad y=\frac{v-u}{4}$.
...
The region in $uv$-coordinates becomes as follows. Calculate the Jacobian determinant and rewrite the integral in $(1)$.
...
\begin{align*}{I}&=\iint_R(2x-4y)\cdot{e}^{x+2y}\,dA=\int_{-2}^2\int_{-2}^22u{e}^{v}\cdot\left|\frac{\partial(x,y)}{\partial(u,v)}\right|\,du\,dv=\int_{-2}^2\int_{-2}^22u{e}^{v}\cdot\left|\frac14\right|\,du\,dv\\\\&=\int_{-2}^2{e}^v\left[\frac{u^2}4\right]_{u=-2}^{u=2}\,dv=\int_{-2}^2{e}^v\cdot0\,dv=\boxed0\end{align*}
