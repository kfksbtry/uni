...
\textbf{1.} Sketch the traces of the following surfaces with the coordinate planes $x=0,\:y=0$, and $z=0$, and then sketch the graphs of them.

\textbf{(a)} $\displaystyle\frac{x^2}{16}+\frac{y^2}4-z^2=1$

\textbf{(b)} $\displaystyle z=\frac{x^2}4+\frac{y^2}4-6$

\vspace{1em}

\textbf{2.} Show that the limit
...
\textbf{3.} Find the equation of the plane passing through the point $P_0(0,1,2)$ and which is perpendicular to the line that is tangent to the curve of intersection of the surfaces
...
\textbf{4.} Find $\displaystyle\frac{\partial w}{\partial s}$ where $w=xy\ln\left(1+\sqrt{x^2+y^2}\right)+xz$ and $x=t+s,\:y={e}^s,\:z=\ln\left(s^2+t\right)$.

\vspace{1em}

\textbf{5.} Use incremental approximation to estimate the value $\tan\left((0.97)\cdot(2.05)^2\right)$.

\vspace{1em}

\textbf{6.} Find the direction vector in which the function
...
\textbf{7.} Find the absolute extrema of
...
\begin{center}
\textbf{2022-2023 Spring Midterm (08/05/2023) Solutions\\
...
\textbf{2.} Apply the Two-Path Test.
...
\textbf{3.} Let $f(x,y,z)=xz^2-2xy+y^2=2$ and $g(x,y,z)=xz-x^2y+z^2=1$ be the level surfaces. The cross product of the gradient of these functions give us the vector that is parallel to the line of intersection. Compute the gradients.
...
\begin{align*}\frac{\partial w}{\partial s}=&\left[y\ln\left(1+\sqrt{x^2+y^2}\right)+xy\cdot\frac1{1+\sqrt{x^2+y^2}}\cdot\left(\frac{x}{\sqrt{x^2+y^2}}\right)+z\right]\cdot1\\\\&+\left[x\ln\left(1+\sqrt{x^2+y^2}\right)+xy\cdot\frac1{1+\sqrt{x^2+y^2}}\cdot\left(\frac{y}{\sqrt{x^2+y^2}}\right)\right]\cdot{e}^s+\frac{2xs}{s^2+t}\end{align*}

Write in terms of $s$ and $t$ rigorously.

\[\boxed{\begin{array}{ll}\dfrac{\partial w}{\partial s}=&
{e}^s\ln\left(1+\sqrt{(t+s)^2+{e}^{2s}}\right)+\dfrac{(t+s)^2\cdot{e}^s}{\sqrt{(t+s)^2+{e}^{2s}}+(t+s)^2+{e}^{2s}}+\ln\left(s^2+t\right)\\\\&+(t+s){e}^s\ln\left(1+\sqrt{(t+s)^2+{e}^{2s}}\right)+\dfrac{(t+s)\cdot{e}^{3s}}{\sqrt{(t+s)^2+{e}^{2s}}+(t+s)^2+{e}^{2s}}\\\\&+\dfrac{2s(t+s)}{s^2+t}
\end{array}}\]

\vspace{1em}

\textbf{5.} Let $f(x,y)=\tan\left(xy^2\right)$. The total differential of $f$ is

\[df=f_x\,dx+f_y\,dy=\sec^2\left(xy^2\right)\cdot y^2\,dx+\sec^2\left(xy^2\right)\cdot2xy\,dy\]

Since $x=0.97\approx1$ and $y=2.05\approx2$, we may approximate the value of $\tan\left(0.97\cdot2.05^2\right)$ near $\tan\left(1\cdot 2^2\right)=\tan4$. Take $x=1,\:y=2,\:dx=0.97-1=-0.03,\:dy=2.05-2=0.05$.
...
\textbf{6.} The function $f$ has the minimum rate of change if the gradient vector of $f$ and the unit direction vector $\mathbf u$ are antiparallel. That is, they have opposite directions.
...
\textbf{7.} $f$ is continuous on a bounded and closed set on $\mathbb R$. By the Extreme Value Theorem, the extrema must exist in the region or on the boundary.
