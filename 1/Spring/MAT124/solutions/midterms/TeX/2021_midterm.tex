...
\begin{center}
\textbf{2020-2021 Spring \\MAT124 Midterm\\(28/04/2022)}
\end{center}

\textbf{1.} Consider the straight lines $\displaystyle x-1=\frac z3,\: y=0$ and $\displaystyle x=\frac{y+1}2=z$.

\textbf{(a)} Show that the lines are skew.

\textbf{(b)} Find an equation for the plane passing through the point $(1,3,5)$ which is parallel to the skew lines.

\vspace{1em}

\textbf{2.} Sketch the graphs of the following surfaces in $\mathbb{R}^3$.
...
\textbf{3.} Let $C$ be the space curve given by the vector-valued function
...
\textbf{4.} Use the $\epsilon-\delta$ definition and show that the function
...
\textbf{5.} Calculate $\displaystyle\frac{\partial w}{\partial u}$ if $\displaystyle w=f(x,y,z)=\frac{x+\sin y}z$ and

\[x=x(u,v)=\ln(u+v),\quad y=y(u,v)={e}^{uv},\quad z=z(u,v)=\frac1u.\]

\vspace{1em}

\textbf{6.} The opposite and adjacent sides of a base and the height of a right triangular prism are measured, and measurements are known to have errors of at most $0.4$ cm. If the opposite and adjacent sides of the base and the height are taken to be $5$ cm, $3$ cm, and $6$ cm, respectively, find the bounds for the propagated error in the volume of the triangular prism.

\vspace{1em}

\textbf{7.} Find the maximum and minimum values of $f(x,y,z)=x-z$ on the ellipsoid

\[x^2+2y^2+z^2=1.\]

\vspace{1em}

\textbf{8.} Find the critical points of $f(x,y)=x^3-y^3+xy$ and classify each point as a relative maximum, a relative minimum, or a saddle point.

\newpage

\begin{center}
\textbf{2020-2021 Spring Midterm (28/04/2021) Solutions\\
(Last update: 28/08/2025 14:04)}
\end{center}

\textbf{1. (a)} Two lines are skew if they do not intersect and are not parallel.
...
\textbf{(b)} From (a), we have the normal vector $\mathbf n$ of the plane. The equation for the tangent plane with the normal vector $\mathbf n$ containing the point $P_0(x_0,y_0,z_0)$ is given by $n_1(x-x_0)+n_2(y-y_0)+n_3(z-z_0)=0$.
...
\textbf{4.} The value of the function at the point $(1,-2)$ is $4$. Therefore, we will show that the limit is also $4$. For every $\epsilon>0$, there exists a $\delta>0$ such that
...
Let $\delta=\dfrac\epsilon3$.

\[\left|\frac{4x^2-y}{2x+y}-4\right|\leq2|x-1|+|y+2|\leq3\cdot\frac\epsilon3=\epsilon\]

Since the limit is equal to the value of the function at $(-1,2)$, $f$ is continuous at $(-1,2)$.

\vspace{1em}

\textbf{5.} Apply the chain rule.
...
\begin{align*}\frac{\partial w}{\partial u}&=\frac1z\cdot\frac1{u+v}+\frac{\cos y}z\cdot v{e}^{uv}+\frac{x+\sin y}{z^2}\cdot\frac1{u^2}\\\\&=\boxed{\frac u{u+v}+{e}^{uv}uv\cos\left({e}^{uv}\right)+\ln(u+v)+\sin\left({e}^{uv}\right)}\end{align*}

\vspace{1em}

\textbf{6.} The volume of a right triangular prism with opposite and adjacent sides and height $a=5,\:b=3,\:h=6$ is given by
\[V(a,b,h)=\frac12 abh\]
...
Given that $|da|\leq0.4,\:|db|\leq0.4,\:|dh|\leq0.4$. Calculate the bounds for the propagated error.
\[\left|dV\right|\leq9\cdot0.4+15\cdot0.4+\frac{15}2\cdot0.4=\boxed{\frac{63}5}\]

\vspace{1em}

\textbf{7.} Let $g(x,y,z)=x^2+2y^2+z^2-1$ be the constraint. Solve the system of equations below.
...
Compare the values $\displaystyle f(0,0,0),\:f\left(\frac1{\sqrt2},0,-\frac1{\sqrt2}\right),\:f\left(-\frac1{\sqrt2},0,\frac1{\sqrt2}\right)$.

\[\boxed{\text{The minimum value is }{-\sqrt2},\text{ the maximum value is }\sqrt2.}\]

\vspace{1em}

\textbf{8.} To identify the critical points, find where both $f_x=f_y=0$ or one of the partial derivatives does not exist.
