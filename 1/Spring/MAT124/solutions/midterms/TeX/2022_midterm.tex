...
\textbf{1.} Find the convergence set of the power series $\displaystyle\sum_{k=1}^\infty\frac{(\ln k)x^k}k$.

\vspace{1em}

\textbf{2.} Show that the vectors $\mathbf{u}=\mathbf{i}+3\mathbf{j}+\mathbf{k},\:\mathbf{v}=2\mathbf{i}-\mathbf{j}-\mathbf{k}$, and $\mathbf{w}=7\mathbf{j}+3\mathbf{k}$ are coplanar (all in the same plane).

\vspace{1em}

\textbf{3.} Find the point where the line
...
\textbf{4.} Sketch the graphs of the following surfaces.
...
\textbf{5.} Let $f$ be the function $\displaystyle f(x,y)=\frac{xy^2}{x^2+y^4}$ for $(x,y)\neq(0,0)$. Show that $f$ has no limit at $(0,0)$.

\vspace{1em}

\textbf{6.} Van der Waal's equation in physical chemistry states that a gas occupying volume $V$ at temperature $T$ (Kelvin) exerts pressure $P$, where

\[\left(P+\frac A{V^2}\right)(V-B)=kT\]

for physical constants $A, B$ and $k$. Compute the following rates.

\textbf{(a)} the rate of change of volume with respect to temperature.

\textbf{(b)} the rate of change of pressure with respect to volume.

\vspace{1em}

\textbf{7.} Consider the surface $z=x^2+y^2-xy$, a paraboloid, on which a particle moves with $x$ and $y$ and coordinates given by $x=\cos t$ and $y=\sin t$. Using the chain rule, find $\displaystyle\frac{dz}{dt}$ when $t=0$. 

\vspace{1em}

\textbf{8.} Find the parametric equations of the tangent line to the curve of intersection of the paraboloid $z=x^2+y^2$ and the ellipsoid $2x^2+2y^2+z^2=8$ at $P=(-1,1,2)$.

\newpage

\begin{center}
\textbf{2021-2022 Spring Midterm (20/04/2022) Solutions\\
(Last update: 26/08/2025 23:42)}
\end{center}

\textbf{1.} Apply the Ratio Test for absolute convergence, and apply other tests at the endpoints.
...
Take the corresponding function $f(x)=\dfrac{\ln x}x$. The function is continuous and positive for $x>1$. It is also decreasing for $x>{e}$ because $x$ grows faster than $\ln x$. Confirm the behavior by taking the first derivative.
...
Since the integral diverges, by the Integral Test, the series $\displaystyle\sum_{k=1}^{\infty}\frac{\ln k}k$ also diverges. Try $x=-1$.
\[x=-1\implies \sum_{k=1}^{\infty}\frac{(\ln k)\cdot(-1)^k}{k}\]

This is an alternating series. The non-alternating part, which is $\dfrac{\ln k}k$, is nonincreasing for $k>{e}$ and it is positive. The previous cases are already confirmed for $x=1$. The limit at infinity is $0$. By Leibniz's Alternating Series Test, the series converges.

Thus, the convergence set for the power series is $\boxed{[-1,1)}$.

\vspace{1em}

\textbf{2.} If $\mathbf{w}\cdot(\mathbf{u}\times\mathbf{v})=0$, the vectors are coplanar. Because $\mathbf{u}\times\mathbf{v}$ is perpendicular to $\mathbf u$ and $\mathbf v$, and the resulting vector is also perpendicular to $\mathbf w$. Compute the cross product.
...
\textbf{3.} For $y=3$, $2y-z=5\implies6-z=5\implies z=1$. For $z=1$, $x-1=z\implies x-1=1\implies x=2$. Therefore, the point where the line and the plane intersect is $\boxed{(2,3,1)}$.
...
\textbf{5.} Apply the Two-Path Test.
...
\textbf{6. (a)} Apply the product rule.
...
(b) Apply the product rule again.
...
\textbf{7.} $z=z(x,y),\:x=x(t)$ and $y=y(t)$. Apply the chain rule.
...
\textbf{8.} Let $f(x,y,z)=x^2+y^2-z=0$ and $g(x,y,z)=2x^2+2y^2+z^2=8$ be level surfaces. The tangent line is perpendicular to both $\nabla f$ and $\nabla g$ at $P$.
