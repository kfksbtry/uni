...
\textbf{1.} Sketch the graph of $f(x)=\ln\left(x^2+1\right)$.

\vspace{1em}

\textbf{2.} Determine the area of the region bounded by $y=4x+3$ and $y=6-x-2x^2$.

\vspace{1em}

\textbf{3.} Use the Washer Method to find the volume of the solid obtained by revolving the region bounded by $y=2\sqrt{x-1},\: y=x-1$ about the line $x=-1$, respectively.

\vspace{1em}

\textbf{4.} Use the Cylindrical Shell Method to find the volume of the solid obtained by revolving the region bounded by $x=y^2-4$ and $x=6-3y$ about $y=-8$.

\vspace{1em}

\textbf{5.} Evaluate the following integrals.

\textbf{(a)} $\displaystyle\int4\left(\frac1x- e^{-x}\right)\cos\left( e^{-x}+\ln x\right)\,dx$

\textbf{(b)} $\displaystyle\int\frac{x^3+10x^2+3x+36}{(x-1)^2\,\left(x^2+4\right)^2}\,dx$

\textbf{(c)} $\displaystyle\int\frac{\sqrt{25x^2-4}}{x}\,dx$

\textbf{(d)} $\displaystyle\int x^2\cos(4x)\,dx$

\textbf{(e)} $\displaystyle\int\frac{dx}{2x^2-3x+2}$

\vspace{1em}

\textbf{6.} Investigate the convergence of the improper integral $\displaystyle\int_{-\infty}^0(1+2x)\, e^{-x}\,dx$.

\vspace{1em}

\textbf{7.} Evaluate the arc length $\displaystyle x=\frac23(y-1)^{3/2}$ for $1\leq y\leq2$.

\newpage

\begin{center}
\textbf{2014-2015 Summer Final (23/08/2015) Solutions\\
(Last update: 20/10/2025 23:04)}
\end{center}

\textbf{1.} $\ln x$ is defined for $x>0$. Therefore, $\ln\left(x^2+1\right)$ is defined on $\mathbb{R}$ because $x^2+1\geq1>0$.
...
\textbf{5. (a)} Use the $u$-substitution method. Let $u= e^{-x}+\ln x$. Then $\displaystyle du=\left(- e^{-x}+\frac1x\right)\,dx$.

\begin{align*}\int4\left(\frac1x- e^{-x}\right)\cos\left( e^{-x}+\ln x\right)\,dx&=\int4\cos u\,du=4\sin u+c\\\\&=\boxed{4\sin\left( e^{-x}+\ln x\right)+c,\:c\in\mathbb{R}}\end{align*}

\textbf{(b)} Decompose the fraction into multiple partial fractions. Let $A,\:B,\:C,\:D,\:E,\:F\in\mathbb{R}$.

\begin{align*}I=\int\frac{x^3+10x^2+3x+36}{(x-1)^2\,\left(x^2+4\right)^2}\,dx=\int\left(\frac{A}{x-1}+\frac{B}{(x-1)^2}+\frac{Cx+D}{x^2+4}+\frac{Ex+F}{(x^2+4)^2}\right)\,dx\end{align*}

Let $N=x^3+10x^2+3x+36$.
...
From $(1)$, $A=-C$. Rewrite $C$ in terms of $A$ and rearrange the equations.
...
From $(2)$, $A+B=-D$. Rewrite $D$ in terms of $A$ and $B$ and rearrange the equations.
\begin{align}
5A+2B+E&=1\\
-5A+3B+F-2E&=10\\
20A+8B-2F+E&=3\\
...
By using the couples $(3)\And(4)$ and $(4)\And(5)$, eliminate $E$.
...
Therefore, $\displaystyle C=\frac{14}{25},\:D=-\frac{36}{25}$. From $(3)$, $\displaystyle E=-\frac15$, and from $(6)$, $\displaystyle F=\frac45$.

Substitute the values into $A,\:B,\:C,\:D,\:E,\:F$.

\begin{equation}I=\int\left(-\frac{14}{25}\cdot\frac1{x-1}+\frac2{(x-1)^2}+\frac{\frac{14}{25}x-\frac{36}{25}}{x^2+4}+\frac{-\frac15x+\frac45}{\left(x^2+4\right)^2}\right)\,dx\end{equation}

From now on, integrate term by term. Integrate the first term in $(7)$.

\begin{equation}\int-\frac{14}{25}\cdot\frac1{x-1}\,dx=-\frac{14}{25}\int\frac1{x-1}\,dx=-\frac{14}{25}\ln\left|x-1\right|+c\end{equation}

Integrate the second term in $(7)$.

\begin{equation}\int\frac{2}{(x-1)^2}\,dx=-\frac2{x-1}+c\end{equation}

Integrate the third term in $(7)$.
...
First, solve the integral on the left in $(11)$. Let $x=2\tan u$, then $dx=2\sec^2u\,du$.
...
Since $x=2\tan u$, $\displaystyle\tan u=\frac x2$
...
Eventually, using $(8),\:(9),\:(10)$ and $(14)$, rewrite $(7)$.

\[I=\boxed{-\frac{14}{25}\ln\left|x-1\right|-\frac2{x-1}+\frac7{25}\ln\left(x^2+4\right)-\frac{67}{100}\arctan\frac x2+\frac{x+1}{10\left(x^2+4\right)}+c,\ c\in\mathbb{R}}\]

\textbf{(c)} Let $\displaystyle x=\frac25\sec u$ for $\displaystyle 0\leq u<\frac\pi2$, then $dx=\displaystyle\frac25\sec u\tan u\,du$.

\[I=\int\frac{\sqrt{25x^2-4}}{x}\,dx=\int\frac{\sqrt{4\sec^2u-4}}{\frac25\sec u}\cdot\frac25\sec u\tan u\,du\qquad\left[\tan^2u+1=\sec^2u\right]\]

\newpage

\begin{align*}I&=2\int\left|\tan u\right|\tan u\,du\qquad\left[\tan u>0\right]\\\\&=2\int\tan^2u\,du=2\int\sec^2u\,du-2\int\,du=2\tan u-2u+c\end{align*}
...
\]
\begin{align*}I&=\frac{x^2}4\sin(4x)-\frac12\left[\frac{-x}4\cos(4x)-\int-\frac14\cos(4x)\,dx\right]\\\\&=\boxed{\frac{x^2}4\sin(4x)+\frac x8\cos(4x)-\frac1{32}\sin(4x)+c,\quad c\in\mathbb{R}}\end{align*}
...
Let $\displaystyle u=\frac{2\sqrt2}{\sqrt7}\left(\sqrt2x-\frac{3\sqrt2}4\right)$, then $\displaystyle du=\frac4{\sqrt7}\,dx$.
...
\textbf{6.} Use the method of integration by parts.
...
\begin{align*}\int_{-\infty}^0(1+2x)\, e^{-x}\,dx&=\lim_{R\to-\infty}- e^{-x}(1+2x)\bigg|_R^0-\int_{-\infty}^0- e^{-x}\cdot2\,dx\\\\&=\lim_{R\to-\infty}\left(- e^0\cdot1+\mathrm{}e^{-R}(1+2\cdot R)\right)+\lim_{P\to-\infty}2 e^{-x}\bigg|_P^0\\\\&=-\infty+2\lim_{P\to-\infty}\left( e^0- e^{-P}\right)=-\infty-\infty=\boxed{-\infty}\end{align*}

The integral diverges to negative infinity.

\vspace{1em}

\textbf{7.} The length of a curve defined by $x=f(y)$ whose derivative is continuous on the interval $a\leq y\leq b$ can be evaluated using the integral
...
Set $a=1,\:b=2$ and find the length.
