...
\textbf{1.} Consider the region $R$ bounded by the curve $y=x^3$, and the straight lines $y=-x$ and $y=x+6$.

\textbf{(i)} Write down the integral corresponding to the area of $R$ with respect to $x$.

\textbf{(ii)} Write down the integral corresponding to the area of $R$ with respect to $y$.

\vspace{1em}

\textbf{2.} Consider the solid $S$ obtained by revolving the region in the first quadrant bounded by $x=-y^2+1,\:y^2=x$ and $y=1/2$ about $x=-1$.

\textbf{(i)} Using the Shell Method, write down the integral corresponding to the volume of the solid $S$.

\textbf{(ii)} Using the Washer Method, write down the integral corresponding to the volume of the solid $S$.

\vspace{1em}

\textbf{3.} Evaluate the following integrals.

\textbf{(a)} $\displaystyle\int\frac{dx}{x^{2/3}\left(\sqrt[3]{x}+4\right)}$

\textbf{(b)} $\displaystyle\int(\ln x)^2\,dx$

\textbf{(c)} $\displaystyle\int\frac{dx}{\sqrt{3+x^2}}$

\textbf{(d)} $\displaystyle\int\frac{dx}{2+\sin x}$

\vspace{1em}

\textbf{4.} Using the Monotone Convergence Theorem, show that the sequence  $\displaystyle\left(\frac{n^2+1}{n^3}\right)_{n\in\mathbb{N}}$ is convergent.

\vspace{1em}

\textbf{5.} Use the Integral Test to determine whether the series $\displaystyle\sum_{k=1}^{\infty}\frac1{k^4+k^2}$ converges or diverges.

\vspace{1em}

\textbf{6.} Find the convergence set for the power series  $\displaystyle\sum_{k=1}^\infty\frac{(x+1)^k}{2k}.$

\vspace{1em}

\textbf{7.} Using a Maclaurin series, show that $\displaystyle\lim_{x\to0}\frac{\sin x}x=1$.

\newpage

\begin{center}
\textbf{2020-2021 Fall Final (18/01/2021) Solutions\\
...
\textbf{3. (a)} Let $u=\sqrt[3]{x}+4$, then $du=\dfrac1{3x^{2/3}}\,dx$.
...
\textbf{(b)} Apply integration by parts.
...
\textbf{(c)} Let $x=\sqrt3\tan u$ for $0<u<\dfrac\pi2$, then $dx=\sqrt3\sec^2u\,du$.
...
\textbf{(d)} We may utilize the tangent half-angle substitution, which is also called the Weierstrass substitution. Let $\displaystyle t=\tan\left(\frac x2\right)$. After some mathematical operations, we get the following.
...
\textbf{4.} Take $f(x)=\dfrac{x^2+1}{x^3}$. We have $f'(x)=-\dfrac{x^2+3}{x^4}<0$ for all $x\geq1$. That means $f$ is decreasing for $x\geq1$. We also have
...
\textbf{6.} Apply the Ratio Test for absolute convergence, and apply other tests at the endpoints.
...
\textbf{7.} The Maclaurin series of $\sin x$ is
